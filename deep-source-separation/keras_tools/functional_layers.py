...
def conv1d(num_feat, kernel_size, stride=1, dilate=1, activation=None, use_bias=True, padding='same', *args, **kwargs):
   return fun._ >> L.Conv1D(
      ...
      #kernel_initializer=keras.layers.initializers.VarianceScaling(),
      *args,
      **kwargs
   )
...
def conv1d_transpose(filters, kernel_size, strides=2, padding='same', *args, **kwargs):
   def impl(x):
      x = XL.AppendDimension(axis=-3)(x)
      x = L.Conv2DTranspose(filters=filters,
         kernel_size=(1, kernel_size), strides=(1, strides),
         #kernel_initializer=keras.layers.initializers.VarianceScaling(),
         padding=padding, *args, **kwargs)(x)
      x = XL.RemoveDimension(axis=-3)(x)
      return x
   return fun._ >> impl
