...
def down_sample(generator, factor=2):
   return LazyGenerator(lambda n: generator(factor*n)[::factor])

def sig_diff(generator):
   return LazyGenerator(lambda n: np.diff(generator(n+1)))
...
rossler = down_sample(LazyGenerator(lambda n: TS.rossler(n, [1,0,0])[::1]), 10)  # down sample to get time scale of Lorenz
funnel = down_sample(LazyGenerator(lambda n: TS.rossler_funnel(n, [1,0,0])[::1]), 10)  # down sample to get time scale of Lorenz
